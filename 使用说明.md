...
| `tfls_metadata.py` | TFLs 页面「Metadata Setup」弹窗（运行 `30_generate_tflmeta_call.sas`） |
...
在 **TFLs** 页面点击「Metadata Setup」按钮，弹出与 Batch Run 第一步风格类似的对话框：

- 默认 SAS 路径：前四个下拉框拼接 + `\utility\tools\30_generate_tflmeta_call.sas`
- **浏览...** / **编辑**：选择或打开上述程序
- **运行 TFL Meta 生成**：在 Linux SAS 服务器上执行该程序；完成后按日志审阅窗口查看 ERROR/WARNING（与「生成PDT」执行 SAS 的方式一致）
...
4. **生成 PDT / Metadata Setup**（可选）：切换到 TFLs 页面，点击「生成PDT」或「Metadata Setup」（运行 `30_generate_tflmeta_call.sas`），按弹窗提示操作
